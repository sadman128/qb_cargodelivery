	...
	while true do
		...
		local v = Config.CargoProviderLocation 
			if(Vdist(v.x, v.y, v.z, pos.x, pos.y, pos.z) < 2.0)then
                ...
                --exports['qb-core']:DrawText('Press E to start Cargo Delivery', 'left')
...
function AlertThePolice()
	...
	if event_is_running then
		...
		QBCore.Functions.TriggerCallback('kenji_alertcargo', function()
...
function DeliverCargo()
	...
	QBCore.Functions.TriggerCallback('esx_cargodelivery:sellCargo', function(sold)
			if sold then
...
function SpawnCargoVehicle(scenario)
    ...
    TriggerEvent("vehiclekeys:client:SetOwner", QBCore.Functions.GetPlate(veh))
			exports[Config.Fuel]:SetFuel(event_vehicle, 100.0)
...
function PurchaseCargo(scenario)
	...
	QBCore.Functions.TriggerCallback('esx_cargodelivery:getCopsOnline', function(police)

		police = police 

		if police >= Config.Scenarios[scenario].MinCopsOnline then
			
			QBCore.Functions.TriggerCallback('esx_cargodelivery:buyCargo', function(bought)
			if bought then
